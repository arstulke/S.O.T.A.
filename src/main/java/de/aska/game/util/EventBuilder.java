...
public class EventBuilder {
    ...
    static de.aska.game.event.Event buildEvent(String line) {
    ...
    private static de.aska.game.event.Event buildThinEvent(Rectangle triggerArea, boolean repeatable, List<String> parameters) {
        String typeName = parameters.get(0).toLowerCase();
        de.aska.game.event.Event.Type type = de.aska.game.event.Event.Type.find(typeName);
        parameters = parameters.subList(1, parameters.size());
        Point target;
        if (type != null) {
            switch (type) {
                ...
                case EXECUTE_CONDITION:
                    ...
                    de.aska.game.event.Event event = buildChainedEvent(parameters);
    ...
    private static de.aska.game.event.Event buildChainedEvent(List<String> parameters) {
        List<String> params;
