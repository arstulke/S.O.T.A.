...
import de.aska.game.event.CheckpointEvent;
...
/**
 * Created Game.java in de.aska.network
 * by Arne on 11.01.2017.
 */
public class Game implements Cloneable {

    private final CoordinateMap<Block> blockMap;
    private final CoordinateMap<List<de.aska.game.event.Event>> eventMap;
    ...
    private final Set<de.aska.game.event.Event> executedEvents = new HashSet<>();
    ...
    private Game(Player player, CoordinateMap<Block> blocks, CoordinateMap<List<de.aska.game.event.Event>> events, GameRenderer gameRenderer) {
        this.player = player;
    ...
    public List<de.aska.game.event.Event> getEvents(Point point) {
    ...
    public void respawn(Session session) {
        ...
        Set<de.aska.game.event.Event> executedEvents = this.executedEvents.stream().filter(event -> event instanceof CheckpointEvent).collect(Collectors.toSet());
    ...
    public boolean isEventExecuted(de.aska.game.event.Event event) {
        return executedEvents.contains(event);
    }

    public void executedEvent(de.aska.game.event.Event event) {
    ...
    public static class Builder {
        ...
        private final CoordinateMap.Builder<List<de.aska.game.event.Event>> events;
        private final GameRenderer.Builder gameRenderer;
        private final Set<String> resources;
        private final boolean verified;
        private boolean textures = true;

        public Builder(Player.Builder playerBuilder, CoordinateMap.Builder<Block> blocks, CoordinateMap.Builder<List<de.aska.game.event.Event>> events, GameRenderer.Builder gameRenderer, String title, String token, Set<String> resources) {
